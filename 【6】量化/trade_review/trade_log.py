...
from config import DATA_DIR, TRADES_FILE
...
class Trade:
    """单笔交易记录"""
    交易编号: str = ""
    日期: str = ""                          # 入场日期 YYYY-MM-DD
    股票代码: str = ""
    股票名称: str = ""
    账户类型: str = ""                      # 核心/产业/事件/实验
    风险簇: str = ""                        # 创新药/AI算力/半导体等
    入场系统: str = ""                      # S1-A/S2-A/预埋
    核心逻辑: str = ""
    入场价: float = 0.0
    止损价: float = 0.0
    风险率: float = 0.0                     # 单笔风险占账户比例 (%)
    股数: int = 0
    仓位金额: float = 0.0
    ...
    是否系统内交易: bool = True
    # 扩展字段（自动计算或手动填写）
    MFE: Optional[float] = None             # 最大有利波动 (R)
    MAE: Optional[float] = None             # 最大不利波动 (R)
    R倍数: Optional[float] = None           # 最终 R 倍数
    备注: str = ""
