...
- **纪律审计**：持仓监控（止损/退出警报、移动止盈建议）、交易日志、入场合规闸门（含六条硬规则行为门禁）、行为纪律自动审计（7 条规则）、周/月复盘报告、信号验证（含数据缺失披露与入场形态双口径，2026-09 可信度修复）
...
└── Invest/                     # ★ 全部代码在此，所有命令须在此目录下运行
    ...
    │   ├── signal_tracker.py   # 信号追踪：突破信号入库（含市场状态/滤网/附注）、每日结算（缺K线补抓+超龄「数据缺失」关闭）、胜率/平均R + 市场状态/入场形态分层（一字板/涨停收盘/非涨停，决策看非涨停组）
    │   └── run_daily.py        # CLI 入口
    ├── research/               # AI 研究助手
    ...
    │   ├── dragon_reasoner.py    # 龙头深度推理（Kimi K2.6，task_type=reasoning，防编造护栏）
    │   ├── event_calendar.py     # 事件日历：财报预约/解禁 + K2.6 联网探查板块未来30天事项（当日缓存）
    │   └── evidence_chain.py     # 个股证据链深挖：引爆点/推导链/正反证据/关联个股（K2.6 联网，Top3，当日缓存）
...
python pipeline/auction_check.py       # 手动跑一次 9:25 竞价判定（交易日 9:26 后；服务已有工作日定时）
...
其他环境变量：`ACCOUNT_EQUITY`（默认 32500，3.25 万实盘）、`DRAWDOWN_STATE`（Normal/Caution/Defensive/Review；未显式设置时由 `review/monitor.py` 按权益曲线自动推导，冲突时以推导值为准）、`WEEKLY_REVIEW_TIME`（15:45，周五自动周报）、`MONTHLY_REVIEW_TIME`（16:00，月末自动月报）、`AUCTION_CHECK_TIME`（09:26，工作日定时竞价判定，`AUCTION_CHECK_ENABLED=false` 关闭）、`SERVER_PORT`（8900）等，完整列表见 `Invest/README.md` 与 `Invest/ARCHITECTURE.md` 第 7 章。
...
- 框架：pytest，目录 `Invest/tests/`，共 **554 个用例**（指标 16 + 合规 12+11 + 持仓 8 + 监控 30+3 + 入场合规闸门 46 + 信号追踪 30 + 策略参数 19 + 回测 12+3 + 热点池 9 + 券商导入 14 + 纪律审计 14+7 + 卖点检查 9 + 统计口径 3 + 热点规则 17 + 催化分析 18 + 趋势池 10 + 滤网 17 + 加仓 13 + 分批退出 8 + 热度门禁 16 + 批量回测 4 + 信号回测对账 1 + 盘前清单 13 + 交易操作层 13 + Web API 10 + 龙头评分 21 + 计划双组 3+3 + K3 推理 15+2 + 计划降级透传 1 + 事件日历 7 + 证据链 9 + THS日线 5 + 行为门禁 14 + 二板战法 46 + 竞价判定 38），已验证全部通过（`554 passed`，2026-09-11 复测）。
