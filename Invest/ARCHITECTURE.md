...
### 13.11 盘前操作清单 + 龙头辨识体系 + Web 控制台（2026-08-06 ~ 08-08 ✅）

**背景**：盘前报告只有候选与警报，缺可执行清单；用户引入《如何识别真假龙头》（三维验证）、《优化方案》（架构A：盘前大模型做逻辑、盘中程序做执行），要求系统自主辨龙头并支持 UI 操作。

| # | 方向 | 实现方式 |
|---|------|----------|
| 1 | 明日操作计划 | `pipeline/daily_plan.py`：滤网全过候选带参考价/止损/股数/闸门预检 + 持仓行动 + 不交易条件；扫描报告第二节 + JSON `daily_plan` 双写；monitor 新增「无止损」高优警报（券商导入缺止损场景实锤） |
| 2 | 龙头辨识体系 | `pipeline/dragon_head.py` 五维量化评分（身位/梯队/强度/逻辑/情绪，S/A/B/C）；`research/dragon_reasoner.py` Kimi 深度推理（task_type=reasoning，防编造护栏：symbol 候选集校验/verdict 枚举/confidence 截断），直出 真龙头/疑似/跟风/伪龙头 + 本期认定 primary + 明日板块聚焦 sector_focus（同一次调用）；涨停归因接入（同花顺 `field=9001` reason_type，逻辑分证据分级 20/12/8/0）；`research/event_calendar.py`（财报预约/解禁 + 联网探查板块未来 30 天事项，NaT 真值陷阱修复）；`research/evidence_chain.py`（Top3 逐股联网深挖：引爆点/推导链/正反证据/关联个股，当日缓存） |
| 3 | Web 控制台 | `web/console.html` 单页（原生 HTML/JS 零依赖）+ `review/trade_ops.py` 程序化操作层 + server.py `/api/*` 端点（写操作 `_trade_lock` 串行，闸门拒绝 409 带违规明细）；建仓/加仓/卖出/卖点检查/报告查看全部可在页面完成；顶栏版本徽标 + 使用说明弹窗（`config.APP_VERSION`） |
| 4 | 数据源与模型 | 个股/指数日线源整体换同花顺 v4/line（新浪/东财本机被风控，重试超时）；推理模型 K3→K2.6（KIMI_MODEL*/KIMI_MODEL_REASONING 默认值，仅允许 temperature=1，llm_client 清单制自动上调）；缓存键含模型名 |
| 5 | 规则变更 | 放开创业板（`BANNED_BOARD_PREFIXES=()` 默认空，机制保留，用户决策 2026-08-07） |
| 6 | 测试 | 全量 427 passed（新增趋势池/滤网/计划/龙头/推理/事件日历/证据链/THS日线/交易操作层/Web API 等测试文件） |

### 13.12 未来扩展开发项（roadmap，按优先级）

| # | 方向 | 价值/依据 | 规模 |
|---|------|-----------|------|
| 1 | **组合级回测**（跨标的资金分配、并发持仓、组合回撤） | 当前只有单标的×批量回测；验证组合层风险预算与回撤状态机的唯一途径 | 大 |
| 2 | **竞价监控与盘中异动提醒**（9:25 竞价报告、QMT 对接） | 《如何识别真假龙头》买入窗口 9:25-9:35；竞价核对清单已离线落地，实时化需盘中数据源 | 中 |
| 3 | **龙虎榜席位评分 / 炸板回封判定** | 五维看盘的两个未量化维度（席位粒度数据、盘中回封过程） | 中 |
| 4 | **HOT-S 信号 × 龙头判定等级相关性统计** | 信号验证闭环：判定（真龙头/疑似/跟风）与实际 5 日结算 R 的相关性，校准评分权重 | 小 |
| 5 | **扫描池扩展**（全市场或 Top N 板块以上） | 当前 Top5 板块成分股 + 热点池；更大覆盖面需评估抓取耗时与风控 | 中 |
| 6 | **GitHub Actions 每日 smoke test**（mock AkShare） | 目前无 CI；427 用例离线可跑 | 小 |
| 7 | **公告向量检索**（announcements 缓存检索） | 公告缓存已积累原文，检索可复用 | 中 |
| 8 | **python-dotenv 启用**（.env 自动加载） | 预留依赖，密钥管理便利化 | 小 |
...
| `MAX_UNITS` / `LOT_SIZE` | 3 / 100 | 最大单位数（V5.0 §5.5 三档）/ 整手 |
...
*文档由源码全量阅读与集成验证生成（2026-07-29 初版，2026-08-08 全面复核更新至 13.11/13.12）。如有代码变更请同步更新本文档。*
