...
# 警报优先级：止损 > 无止损 > 退出 > 接近止损 > 移动止盈 > 移动止损建议（建议类，仅提示不改库）
_ALERT_PRIORITY = {"止损": 0, "无止损": 1, "退出": 2, "接近止损": 3, "移动止盈": 4, "移动止损建议": 5}
...
def _check_single_position(trade: Trade, db_path: Optional[Path] = None) -> dict:
    ...
    if trade.止损价 <= 0:
        ...
        # 移动止盈（六条硬规则④，替代「次日开盘必卖」）：
        # 现价自入场后最高收盘价回落 ≥ TRAILING_PROFIT_PCT 且仍盈利 → 建议类警报，仅提示不改库
        entry_date = str(trade.日期 or "")
        since_entry = df[df["trade_date"].astype(str) >= entry_date] if entry_date else df
        peak = float(since_entry["close"].max()) if not since_entry.empty else close
        trail_ref = round(peak * (1 - TRAILING_PROFIT_PCT / 100), 2)
        if peak > trade.入场价 and close > trade.入场价 and close <= trail_ref:
            info["类型"] = "移动止盈"
            info["建议动作"] = (
                f"现价自入场后最高收盘 {peak:.2f} 回落超 {TRAILING_PROFIT_PCT:.0f}%"
                f"（移动止盈）：建议离场，参考挂单价 {trail_ref:.2f}"
            )
        elif trade.止损价 < trade.入场价:
